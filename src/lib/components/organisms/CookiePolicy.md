	...
	*Effective Date: 8, Sep 2026*
	...
	3. What We Actually Store
		...
		**We do not use any.** There is no Google Analytics, no advertising network, and no social media tracking pixel on this site, and neither of the two analytics in section 3.3 sets a cookie of any kind. PostHog's script is served from langx.io itself rather than from its CDN, the way our fonts are; Cloudflare's is the exception, and it is named below.
		...
		Three things are worth naming rather than hiding behind that sentence. The star count in our header is read from GitHub's public API by your browser, so GitHub sees the request the way it sees any visit to a page. Serving PostHog's script ourselves does not mean nothing reaches PostHog: the page views still end up at its European endpoint, though since September 2026 the ones from this site take a hop through langx.io on the way — your browser sends them to us and our own edge forwards them on, rather than connecting to PostHog itself. We did that for a plain reason: blockers recognise PostHog's address and were dropping page views, so the count was incomplete by an amount we could not see. It is a more complete count and not a different one — nothing changed about what is sent — and we strip any cookie before forwarding, so PostHog receives the same request it would have received directly, minus that. And Cloudflare, which serves this site, adds a small measurement script of its own from its own domain — the one described in section 3.3, injected at its edge rather than written into our pages. None of the three sets a cookie, and none is told anything about you beyond what is described above.
